...
$(function () {
  ...
  $(window).scroll(function () {
    ...
    if (isElementInViewport($('#ending-text-wrapper'))) {
      if (!hasHappened) {
        setTimeout(() => {
          $('#ending-text').css({ 'display': 'block' });
          $('#ending-text-wrapper').addClass('centered-and-still');
          setTimeout(() => {
            $('#ending-text').addClass('fadeOut');
            console.log(hasHappened ? 'paso 1' : 'no paso 1');
            hasHappened = true;
            setTimeout(() => {
              $('#ending-text-wrapper').removeClass('centered-and-still');
              $('#ending-text').css({ 'display': 'none' });
              console.log(hasHappened ? 'paso 2' : 'no paso 2');
            }, 1000);
          }, 3500);
        }, 500);
      }
...
function blink() {
  $('#eye-svg').addClass('blink');
  setTimeout(() => {
    $('#eye-svg').removeClass('blink');
  }, 350);
}
